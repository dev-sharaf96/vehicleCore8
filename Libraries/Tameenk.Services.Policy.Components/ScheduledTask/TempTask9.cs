...
namespace Tameenk.Services.Policy.Components
{
    public class TempTask9 : ITask
    {
        ...
        #region Ctor
        public TempTask9(IPolicyProcessingService policyProcessingService,
        ...
        {
            var policies = _policyContext.GetPolicyInfoForRenewal();
            if (policies == null || policies.Count == 0)
            {
                System.IO.File.WriteAllText(@"C:\inetpub\wwwroot\ScheduledTask\logs\GetPolicyInfoForRenewal.txt", "data is null");
            }

            foreach (var policy in policies)
            {
                _policyContext.HandlePolicyRenewalInfo(policy);
            }
        }
