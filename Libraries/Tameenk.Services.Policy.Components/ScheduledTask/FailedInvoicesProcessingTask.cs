...
{
    public class FailedInvoicesProcessingTask : ITask
    {
        #region Fields
        ...
        #region Ctor
        public FailedInvoicesProcessingTask(IPolicyProcessingService policyProcessingService,
        ...
        {
            var policiesToProcess = _checkoutDetailRepository.Table.Where(a=>a.PolicyStatusId==9).ToList();
            ...
            foreach (var policy in policiesToProcess)
            {
                try
                {
                   ...
                   if(!_policyContext.GnerateInvoiceRecord(policy.ReferenceId, policy.PaymentMethodId.Value,policy.SelectedInsuranceTypeCode.Value,policy.InsuranceCompanyId.Value, out exception))
                    {
                        System.IO.File.WriteAllText(@"C:\inetpub\wwwroot\ScheduledTask\logs\FailedInvoicesProcessingTask_" + DateTime.Now.ToString("dd_MM_yyyy_hh_mm_ss_mms") + ".txt", " Exception is:" + exception.ToString());
                        continue;
                    }

                    if (_policyContext.GenerateInvoicePdf(policy.ReferenceId, out exception))
                    {
                ...
                {
                    _checkoutDetailRepository.Update(policy);
                }
            }
