...
{
    public class PolicyYearlyMaximumPurchaseTask : ITask
    {
        #region Fields
        ...
        #region Ctor
        public PolicyYearlyMaximumPurchaseTask(IPolicyProcessingService policyProcessingService,
        ...
        {
            var policiesToProcess = _policyProcessingService.GetPolicyYearlyMaximumPurchaseQueue(maxTrials);
            //System.IO.File.WriteAllText(@"C:\inetpub\wwwroot\ScheduledTask\logs\PolicyYearlyMaximumPurchaseTask_log_Count" + ".txt", policiesToProcess.Count().ToString());

            foreach (var policy in policiesToProcess)
            {
                try
                {
                    var responseModel = _policyContext.HandlePolicyYearlyMaximumPurchaseTask(policy);
                   // System.IO.File.WriteAllText(@"C:\inetpub\wwwroot\ScheduledTask\logs\PolicyYearlyMaximumPurchaseTask_log_" + policy.ReferenceId + ".txt", responseModel.ErrorDescription);

                }
                catch (Exception exp)
                {
                    System.IO.File.WriteAllText(@"C:\inetpub\wwwroot\ScheduledTask\logs\PolicyYearlyMaximumPurchaseTask_log_" + policy.ReferenceId + ".txt", " Exception is:" + exp.ToString());
                }
            }
        }
