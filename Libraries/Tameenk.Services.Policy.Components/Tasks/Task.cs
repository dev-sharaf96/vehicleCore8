...
namespace Tameenk.Services.Policy.Components
{
    ...
    public partial class Task
    {
        ...
        public void Execute(bool throwException = false, bool dispose = true, bool ensureRunOnOneWebFarmInstance = true)
        {
            ...
            var scheduleTask = scheduleTaskService.GetFromScheduleTask(this.Type);
            if (scheduleTask == null)
            {
                System.IO.File.WriteAllText(@"C:\inetpub\wwwroot\ScheduledTask\logs\scheduleTask_log_" + DateTime.Now.ToString("dd_MM_yyyy_hh_mm_ss_mms") + ".txt", "Task is " + this.Type + " is null ");
                return;
            }
            try
            {
                ...
                //        scheduleTaskService.UpdateScheduleTask(scheduleTask.Id, scheduleTask.LastStart, scheduleTask.LastEnd, scheduleTask.LastSuccess);
                //    }
                //}
                ...
                try
                {
                    ...
                    {
                        System.IO.File.WriteAllText(@"C:\inetpub\wwwroot\ScheduledTask\logs\FailedToCreateTask_log_" + DateTime.Now.ToString("dd_MM_yyyy_hh_mm_ss_mms") + ".txt", "Task is " + this.Type + " failed to create task");
                    }
                }
                catch (Exception ex)
                {
                    System.IO.File.WriteAllText(@"C:\inetpub\wwwroot\ScheduledTask\logs\CreateTask_log_" + DateTime.Now.ToString("dd_MM_yyyy_hh_mm_ss_mms") + ".txt", "Task is " + scheduleTask.Type + " and this.Enabled =" + this.Enabled + " Exception is:" + ex.ToString());
                }
            }
            catch (Exception ex)
            {
                ...
                System.IO.File.WriteAllText(@"C:\inetpub\wwwroot\ScheduledTask\logs\Main_log" + DateTime.Now.ToString("dd_MM_yyyy_hh_mm_ss_mms") + ".txt", "Task is "+ scheduleTask.Type + " and this.Enabled =" + this.Enabled +" Exception is:" + ex.ToString());
                //log error
