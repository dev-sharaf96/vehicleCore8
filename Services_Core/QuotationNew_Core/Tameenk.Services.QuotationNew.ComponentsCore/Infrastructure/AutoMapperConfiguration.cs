    ...
    public static class AutoMapperConfiguration
    {
        public static void Init()
        {
            MapperConfiguration = new MapperConfiguration(cfg =>
            {
                ...
                //cfg.CreateMap<QuotationRequestInfoModel, QuotationNewRequestDetails>(MemberList.None)
                //.ForMember(s => s.SocialStatus, mo => mo.MapFrom(dst => dst.SocialStatus ?? Tameenk.Core.Domain.Enums.Quotations.SocialStatus.NotAvailable));
                cfg.CreateMap<QuotationRequestInfoModel, QuotationNewRequestDetails>(MemberList.None)
                    .ForMember(s => s.CardIdType, opt => opt.Ignore())
                    .ForMember(s => s.InsuredGender, opt => opt.Ignore())
                    .ForMember(s => s.SocialStatus, opt => opt.Ignore())
                    .ForMember(s => s.InsuredEducation, opt => opt.Ignore())
                    .ForMember(s => s.ParkingLocation, opt => opt.Ignore())
                    .ForMember(s => s.AxlesWeight, opt => opt.Ignore())
                    .ForMember(s => s.MileageExpectedAnnual, opt => opt.Ignore())
                    .ForMember(s => s.TransmissionType, opt => opt.Ignore())
                    .ForMember(s => s.VehicleUse, opt => opt.Ignore())
                    .ForMember(s => s.EngineSize, opt => opt.Ignore())
                    .ForMember(s => s.VehicleIdType, opt => opt.Ignore());
            });
