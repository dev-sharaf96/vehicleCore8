...
namespace Tameenk.Services.Checkout.Api.Controllers
{
    public class CheckoutController : BaseApiController
    {
        ...
        [HttpGet]
        public IActionResult CheckoutDetails(string ReferenceId, string QtRqstExtrnlId, string lang, string Channel, string productId = "", string hashed = "", string selectedProductBenfitId = "")
        {
        ...
        public IActionResult SubmitCheckoutDetails()
        {
            CheckoutRequestLog log = new CheckoutRequestLog();
            string userId = _authorizationService.GetUserId(User);
            log.UserId = userId;
        ...
        public IActionResult AddItemToCart([FromBody] Tameenk.Core.Domain.Dtos.AddItemToCartModel model)
        {
            CheckoutRequestLog log = new CheckoutRequestLog();
            string userId = _authorizationService.GetUserId(User);
            log.UserId = userId;
        ...
        [HttpGet]
        public IActionResult HyperpayProcessPayment(string id, string Lang, string Channel)
        {
            string userId = _authorizationService.GetUserId(User);
            Guid uId = Guid.Empty;
        ...
        //            return Ok(output);
        //        }
        //        return Ok(output);        //    }        //    catch (Exception ex)        //    {        //        output.ErrorCode = CheckoutOutput.ErrorCodes.ServiceException;        //        output.ErrorDescription = CheckoutResources.ErrorGeneric;        //        return Ok(output);        //    }        //}        [HttpPost]        [Route("api/Checkout/VerifyCode")]        public IActionResult VerifyCode(string phoneNumber, string code,string channel)        {            string userId = _authorizationService.GetUserId(User);
            CheckoutOutput output = new CheckoutOutput();
            if (string.IsNullOrEmpty(userId))            {                output.ErrorCode = CheckoutOutput.ErrorCodes.AnonymousUser;                output.ErrorDescription = "Anonymous User";
          ...
          //  var language = GetCurrentLanguage(Lang);            var result = _checkoutContext.ManagePhoneVerification(Guid.Parse(userId), phoneNumber, code,channel);            if (!result)            {
            ...
            output.ErrorDescription = "Success";            return Ok(output);        }        [HttpPost]        [Route("api/Checkout/ResendVerifyCode")]        public IActionResult ResendVerifyCode(string userId, string phoneNumber, string lang="ar")        {            var output = _checkoutContext.ResendVerifyCode(userId,phoneNumber,lang);            return Ok(output);        }

        [HttpPost]
        [Route("api/checkout/sendactivationemail")]
        public IActionResult SendActivationEmailToReceivePolicy([FromBody] Tameenk.Core.Domain.Dtos.SendActivationEmailModel model)
        {
            string userId = _authorizationService.GetUserId(User);
            CheckoutOutput output = new CheckoutOutput();
            if (string.IsNullOrEmpty(userId))            {                output.ErrorCode = CheckoutOutput.ErrorCodes.AnonymousUser;                output.ErrorDescription = CheckoutResources.ErrorGeneric;
        ...
        public IActionResult GetAllPaymentMethodByChannel(string channel, bool showWalletPaymentOption = false)
        {
            try
            {
        ...
        public IActionResult PaymentUsingHyperpay(string referenceId, string QtRqstExtrnlId, string productId, string selectedProductBenfitId, string hashed, int paymentMethodCode, string channel, string lang)
        {
            try
            {
                string userId = _authorizationService.GetUserId(User);
        ...
        public IActionResult ProcessApplePayPayment()
        {
            try
            {
                if (Request.Content.IsMimeMultipartContent())
        ...
        public IActionResult ActivateUserEmailToReceivePolicy([FromBody] Tameenk.Core.Domain.Dtos.ActivateUserEmailModel model)
        {
            CheckoutOutput output = new CheckoutOutput();
            string userId = _authorizationService.GetUserId(User);
        ...
        public IActionResult CheckDiscountCode([FromBody] Tameenk.Core.Domain.Dtos.CheckotDiscountModel model)
        {
        ...
        public IActionResult LogApplePayError(string userId, string referenceId, string errorDescription)
        {
            try
            {
        ...
        public IActionResult PrePaymentChecks(PrePaymentCheckModel model)
        {
        ...
        public IActionResult AddIVRItemToChart([FromBody] RenewalAddItemToChartModel model)
        {
